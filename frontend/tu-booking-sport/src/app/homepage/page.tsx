...
export default function HomePage() {
  ...
  useEffect(() => {
    // โหลดทั้งสอง API พร้อมกัน (หรือจะแยกก็ได้)
    const fetchAll = async () => {
      ...
      try {
        ...
        // fetch favorites
        try {
          const favResp = await fetch(`http://localhost:8081/favorite/${userStudentId}`);
          if (!favResp.ok) throw new Error('Failed to fetch favorites');
          const favData = await favResp.json();

          const formatted: Favorite[] = favData.map((fav: any) => ({
            id: fav.favorite_id,
            name: fav.sport_type,
          ...
          // ถ้า favorite ล้ม จะไม่หยุดการโหลด sports — แค่ log และเก็บข้อความ error (หรือเลือกไม่แสดง)
