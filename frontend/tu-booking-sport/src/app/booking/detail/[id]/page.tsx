...
export default function BookingDetailPage({
    ...
    const fetchAndFindBooking = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:8081/MyBookings/${userId}`,
          {
            credentials: "include",
          }
        );
        ...
        const data: {
          current: Omit<BookingItem, "id">[];
          history: Omit<BookingItem, "id">[];
        } = await response.json();
        ...
        if (foundBooking) {
          ...
          throw new Error(
            `Booking with ID ${params.id} not found for this user.`
          );
        }
      } catch (err: any) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <p className="text-gray-500 text-center px-4">
          {error || `Booking with ID ${params.id} could not be loaded.`}
        </p>
  ...
  const formattedTime =
    booking.startTime && booking.endTime
      ? `${booking.startTime.substring(0, 5)} - ${booking.endTime.substring(
          0,
          5
        )}`
      : "N/A";
          ...
          <main className="p-6 font-nunito">
            <div className="rounded-lg border border-gray-200 bg-white p-6 text-center shadow-md">
              ...
              <div className="mt-6 text-left space-y-2 border-t pt-4">
                <h2 className="text-2xl font-bold text-tu-navy">
                  {booking.name}
                </h2>
